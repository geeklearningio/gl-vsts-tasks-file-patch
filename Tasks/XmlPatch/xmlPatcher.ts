...
export class XmlPatcher implements patch.IPatcher {
  ...
  public createNode(
    xml: Document,
    parsedName: { name: string; namespace: string | null; localName: string }
  ) {
    let newNode: Element;
  ...
  private detectArrayOperation(
    ...
    var isLastFragmentDigitOnly: string[] = XRegExp.match(
      lastFragment,
      /^\d+$/g
    );
  ...
  private detectNamespace(
    xml: Document,
    nodeName: string
  ): { name: string; namespace: string | null; localName: string } {
  ...
  private getParentPath(
    path: string
  ): { path: string; nodeName: string; isAttribute: boolean } {
    var lastSlash = path.lastIndexOf("/");
  ...
  private move(xml: Document, select: any, patch: IPatch): boolean {
    ...
    if (
      arrayOperation.isArrayOperation ||
      arrayOperationFrom.isArrayOperation
    ) {
  ...
  private replace(xml: Document, select: any, patch: IPatch): boolean {
    ...
    if (arrayOperation.isArrayOperation) {
      ...
      var newNode = this.createNode(
        xml,
        this.detectNamespace(xml, patch.value)
      );
  ...
  apply(content: string): string {
    ...
    for (var index = 0; index < this.patches.length; index++) {
      ...
      var operation: (xml: any, select: any, patch: Operation) => boolean = (
        xml,
        select,
        patch
      ) => false;
